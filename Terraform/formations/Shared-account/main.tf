          ...
          rule,
          {
            ingress_rules = rule.ingress_rules != null ? [
              for ing_rule in rule.ingress_rules : merge(
                ing_rule,
                {
                  source_vpc_sg_id = rule.source_vpc_sg_key != null ? module.shared_vpc[each.value.vpc_name].security_group[rule.source_vpc_sg_key].id : rule.vpc_security_group_id
                }
              )
            ] : null
          },
          {
            egress_rules = rule.egress_rules != null ? [
              for egr_rule in rule.egress_rules : merge(
                egr_rule,
                {
                  source_vpc_sg_id = rule.target_vpc_sg_key != null ? module.shared_vpc[each.value.vpc_name].security_group[rule.target_vpc_sg_key].id : rule.vpc_security_group_id
                }
              )
            ] : null
          }
