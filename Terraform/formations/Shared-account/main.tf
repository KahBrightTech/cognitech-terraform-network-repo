...
#--------------------------------------------------------------------
module "eks_clusters" {
  source   = "git::https://github.com/njibrigthain100/Cognitech-terraform-iac-modules.git//terraform/modules/EKS-Cluster?ref=v1.4.99"
...
module "eks_cluster_addons" {
  source = "git::https://github.com/njibrigthain100/Cognitech-terraform-iac-modules.git//terraform/modules/EKS-Cluster-Addons?ref=v1.4.98"
  for_each = merge([
    for cluster in var.eks_clusters :
    (cluster.eks_addons != null && lookup(cluster, "create_node_group", true)) ?
    { for addon in cluster.eks_addons : addon.key => merge(addon, { cluster_name = cluster.name }) } :
    {}
  ]...)
  common = var.common
  eks_addons = merge(
