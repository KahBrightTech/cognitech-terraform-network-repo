...
module "transit_gateway_subnet_route" {
  ...
  tgw_subnet_route = {
    route_table_id     = each.value.Is_this_shared_services ? each.value.route_table_id : module.customer_vpc[each.value.vpc_name].private_routes[each.value.subnet_name].private_route_table_id
