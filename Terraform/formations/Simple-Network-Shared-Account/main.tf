  ...
  load_balancer = merge(
    each.value,
    {
      ...
      default_listener = (each.value.create_default_listener == true) ? merge(
        ...
        {
          certificate_arn = lookup(each.value.default_listener, "certificate_arn", null) != null ? each.value.default_listener.certificate_arn : try(module.shared_vpc.certificates[each.value.vpc_name].arn, null)
        }
